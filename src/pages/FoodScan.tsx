...
export default function FoodScan(){
 ...
 const [photo,setPhoto]=useState<string|null>(null); const [result,setResult]=useState<any>(null); const [busy,setBusy]=useState(false); const [error,setError]=useState(''); const [barcodeManual,setBarcodeManual]=useState(''); const [barcodeGrams,setBarcodeGrams]=useState(100); const [detectedFoods,setDetectedFoods]=useState<any[]>([]);
 ...
 const reset=()=>{setPhoto(null);setResult(null);setDetectedFoods([]);setError('')};
 ...
 const addBarcode=async()=>{if(!result||!user)return;const t=result.total||{};const grams=Number(barcodeGrams);if(!Number.isFinite(grams)||grams<=0||grams>5000){setError('Entre une quantité valide en grammes.');return}const ratio=grams/100;const {error:e}=await supabase.from('food_entries').insert({user_id:user.id,meal_type:meal,food_name:result.description||'Produit scanné',calories:Math.round(Number(t.kcal||0)*ratio),protein:Number(t.protein||0)*ratio,carbs:Number(t.carbs||0)*ratio,fat:Number(t.fat||0)*ratio,created_at:new Date().toISOString()});if(e)setError(e.message);else navigate('/fuel')};
 const mealTotals=detectedFoods.length?detectedFoods.reduce((t,x)=>{const ratio=Number(x.grams||0)/Math.max(1,Number(x.baseGrams||1));t.kcal+=Number(x.kcal||0)*ratio;t.protein+=Number(x.protein||0)*ratio;t.carbs+=Number(x.carbs||0)*ratio;t.fat+=Number(x.fat||0)*ratio;return t},{kcal:0,protein:0,carbs:0,fat:0}):result?.total||{kcal:0,protein:0,carbs:0,fat:0};
 const addMeal=async()=>{if(!result||!user)return;if(detectedFoods.some(x=>!String(x.name||'').trim()||!Number.isFinite(Number(x.grams))||Number(x.grams)<=0)){setError('Vérifie le nom et la portion de chaque aliment.');return}const t=mealTotals;const name=detectedFoods.length?detectedFoods.map(x=>x.name).join(', '):(result.description||'Repas scanné');const {error:e}=await supabase.from('food_entries').insert({user_id:user.id,meal_type:meal,food_name:name,calories:Math.round(Number(t.kcal||0)),protein:Number(t.protein||0),carbs:Number(t.carbs||0),fat:Number(t.fat||0),created_at:new Date().toISOString()});if(e)setError(e.message);else navigate('/fuel')};
 ...
 if(mode) return <div style={{minHeight:'100vh',background:BLACK,color:'#fff',display:'flex',flexDirection:'column'}}>
  ...
  <div style={{maxWidth:560,width:'100%',margin:'0 auto',minHeight:'100vh',display:'flex',flexDirection:'column',position:'relative',background:BLACK}}>
   ...
   {result&&!busy&&mode==='qr'&&!result.pending&&<div style={{background:'#fff',color:BLACK,borderRadius:'24px 24px 0 0',padding:20,marginTop:12}}>
    ...
    <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:6,marginTop:10}}>{[['Calories',Math.round(Number(result.total?.kcal||0)*barcodeGrams/100)],['Protéines',Math.round(Number(result.total?.protein||0)*barcodeGrams/100)+'g'],['Glucides',Math.round(Number(result.total?.carbs||0)*barcodeGrams/100)+'g'],['Lipides',Math.round(Number(result.total?.fat||0)*barcodeGrams/100)+'g']].map(([l,v])=><div key={String(l)} style={{background:'#F6F6F6',borderRadius:12,padding:'10px 3px',textAlign:'center'}}><b style={{fontSize:15}}>{v}</b><div style={{fontSize:8.5,color:MUTED,marginTop:3}}>{l}</div></div>)}</div>
    <label style={{display:'flex',alignItems:'center',justifyContent:'space-between',gap:12,marginTop:12,fontSize:11,fontWeight:850}}>QUANTITÉ <span style={{display:'flex',alignItems:'center',gap:6}}><input type="number" min="1" max="5000" value={barcodeGrams} onChange={e=>setBarcodeGrams(Number(e.target.value))} style={{width:82,border:'1px solid '+BORDER,borderRadius:10,padding:'9px 10px',fontWeight:900,textAlign:'right'}}/> g</span></label>
   ...
   {result&&!busy&&(mode==='equipment'||mode==='cardio'||mode==='menu'||mode==='fridge')&&<div style={{background:'#fff',color:BLACK,borderRadius:'24px 24px 0 0',padding:20,marginTop:12}}>
    <div style={{fontSize:10,color:MUTED,fontWeight:900}}>ANALYSE NOX · À VÉRIFIER</div><div style={{fontSize:18,fontWeight:950,marginTop:5}}>{result.activity_type||result.description||result.name||COPY[mode].title}</div>
    {result.confidence&&<div style={{fontSize:11,color:MUTED,marginTop:4}}>Confiance : {String(result.confidence)}</div>}
    <div style={{fontSize:12,color:MUTED,lineHeight:1.5,marginTop:9}}>NOX a préparé cette analyse à partir de la photo. Vérifie toujours les informations avant de les utiliser ou de les enregistrer.</div>
    {mode==='cardio'&&<button onClick={()=>navigate('/body',{state:{machineScan:result,source:'machine_scan'}})} style={{width:'100%',border:0,borderRadius:12,background:LIME,padding:14,fontWeight:950,marginTop:14}}>VÉRIFIER L'ACTIVITÉ</button>}
    {mode==='equipment'&&<button onClick={()=>navigate('/training',{state:{equipmentScan:result}})} style={{width:'100%',border:0,borderRadius:12,background:LIME,padding:14,fontWeight:950,marginTop:14}}>VOIR DANS TRAINING</button>}
    {(mode==='menu'||mode==='fridge')&&<div style={{marginTop:12}}>
      {(advanced.items||advanced.ingredients||advanced.suggestions)&&<div style={{background:'#F6F6F6',borderRadius:12,padding:11,fontSize:11,color:MUTED,lineHeight:1.5}}>{(advanced.items||advanced.ingredients||advanced.suggestions||[]).slice(0,5).map((x:any,i:number)=><div key={i}>• {typeof x==='string'?x:(x.name||x.description||'Élément détecté')}</div>)}</div>}
      <div style={{fontSize:10,color:MUTED,marginTop:10}}>Destination · {meal}</div>
      <button onClick={()=>void addAdvancedMeal()} style={{width:'100%',border:0,borderRadius:12,background:LIME,padding:14,fontWeight:950,marginTop:8}}>{mode==='menu'?'AJOUTER À MA NUTRITION':'AJOUTER LE REPAS SUGGÉRÉ'}</button>
    </div>}
   </div>}
