...
export default function FoodScan(){
 ...
 const addAdvancedMeal=async()=>{if(!user||!result)return;const t=advanced.total||advanced.nutrition||{};const name=advanced.description||advanced.name||(mode==='menu'?'Plat du menu':'Repas suggéré');const {error:e}=await supabase.from('food_entries').insert({user_id:user.id,meal_type:meal,food_name:String(name),calories:Math.round(Number(t.kcal??t.calories??advanced.calories??0)),protein:Number(t.protein??advanced.protein??0),carbs:Number(t.carbs??advanced.carbs??0),fat:Number(t.fat??advanced.fat??0),created_at:new Date().toISOString()});if(e)setError(e.message);else navigate('/fuel')};
 ...
 if(mode) return <div style={{minHeight:'100vh',background:BLACK,color:'#fff',display:'flex',flexDirection:'column'}}>
  ...
  <div style={{maxWidth:560,width:'100%',margin:'0 auto',minHeight:'100vh',display:'flex',flexDirection:'column',position:'relative',background:BLACK}}>
   ...
   {result&&!busy&&(mode==='equipment'||mode==='cardio'||mode==='menu'||mode==='fridge')&&<div style={{background:'#fff',color:BLACK,borderRadius:'24px 24px 0 0',padding:20,marginTop:12}}>
    ...
    {mode==='cardio'&&<button onClick={()=>navigate('/body?add=activity',{state:{machineScan:result,source:'machine_scan'}})} style={{width:'100%',border:0,borderRadius:12,background:LIME,padding:14,fontWeight:950,marginTop:14}}>VÉRIFIER L'ACTIVITÉ</button>}
    ...
    {(mode==='menu'||mode==='fridge')&&<div style={{marginTop:12}}>
      ...
      <button onClick={()=>void addAdvancedMeal()} style={{width:'100%',border:0,borderRadius:12,background:LIME,padding:14,fontWeight:950,marginTop:10}}>{mode==='menu'?'AJOUTER À MA NUTRITION':'AJOUTER LE REPAS SUGGÉRÉ'}</button>
    </div>}
   </div>}
