...
type Step = 'intro' | 'store' | 'prefs' | 'generating' | 'list';
...
Retourne UNIQUEMENT un tableau JSON de 14 à 24 objets: [{"name":"...","qty":"...","category":"Fruits & légumes|Protéines|Féculents|Produits frais|Épicerie|Petit-déjeuner|Autres","note":"..."}]. Quantités réalistes pour ${nextPrefs.people} personne(s) et ${nextPrefs.days} jours.`;
  ...
  const visualStep=step==='intro'?1:step==='store'?2:step==='prefs'?3:4;

  const back=()=>{
    if(step==='intro')navigate(-1);
    else if(step==='store')setStep('intro');
  ...
  return <div className="qg-root">
    <style>{`
      *{box-sizing:border-box} body{margin:0}
      ...
      .stores{display:grid;grid-template-columns:repeat(3,1fr);gap:9px}.store{min-height:103px;border:1px solid ${BORDER};background:#fff;border-radius:16px;padding:10px 6px;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:7px;font-weight:800;font-size:10px;cursor:pointer}.store.active{border:2px solid ${ACCENT};background:#fbfff1}.store img{width:48px;height:48px;object-fit:contain;border-radius:10px}.brandFallback{height:48px;display:grid;place-items:center;font-size:18px;font-weight:950}
    ...
    <main className="qg-main">
      {step==='intro' && <>
        <section className="card hero">
          <div><h1>Des courses simples,<br/>une semaine <span className="lime">plus facile</span></h1><p>Dis-nous juste quelques infos et NOX s’occupe du reste.</p></div>
          <div className="mascot">🤖🛍️</div>
          <div className="benefits"><div className="benefit">⚡<b>Rapide</b>En 1 minute</div><div className="benefit">🎯<b>Adaptée</b>à ton objectif</div><div className="benefit">🌿<b>Sans prise</b>de tête</div></div>
        </section>
        <Primary onClick={()=>setStep('store')}>C’est parti　→</Primary>
        <div style={{textAlign:'center',fontSize:10,color:'#aaa',marginTop:14}}>Une alimentation adaptée. Un toi plus fort.</div>
      </>}
      ...
      {step==='store' && <>
        ...
        <div className="stores">
          {STORES.map(s=><button key={s.name} className={`store ${store.chain===s.name?'active':''}`} onClick={()=>setStore(x=>({...x,chain:s.name}))}>
            <img src={`https://www.google.com/s2/favicons?domain=${s.domain}&sz=128`} alt="" onError={e=>{e.currentTarget.style.display='none'}}/>
            <span style={{color:s.brand}}>{s.name}</span>
          </button>)}
        </div>
      ...
      {step==='prefs' && <>
        ...
        <Card><Title icon="🍽️" title="Combien de temps ?" sub="Choisis la durée et le nombre de personnes."/>
          <div className="two">
        ...
        <Card><Title icon="€" title="Quel budget ?" sub="Le budget guide les choix de produits. Aucun prix inventé."/>
          <div className="choiceGrid">{(['Éco','Équilibré','Confort'] as Budget[]).map((x,i)=><button key={x} className={`choice ${prefs.budget===x?'active':''}`} onClick={()=>setPrefs(p=>({...p,budget:x}))}>{['🌿','⚖️','◇'][i]}　{x}</button>)}</div>
        </Card>
        <Card><Title icon="🌱" title="Quel type d’alimentation ?" sub="Choisis le cadre alimentaire principal."/>
          <div className="choiceGrid">{DIETS.map((x,i)=><button key={x} className={`choice ${prefs.diet===x?'active':''}`} onClick={()=>setPrefs(p=>({...p,diet:x}))}>{['🥩','🌿','🌱','🐟','☾','⊘'][i]}　{x}</button>)}</div>
        </Card>
      ...
      {step==='generating' && <div className="gen">
        <div className="basket">🛒</div><h1>NOX prépare ta liste...</h1>
