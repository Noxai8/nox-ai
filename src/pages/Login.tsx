...
export default function Login() {
  ...
  const handleLogin = async () => {
    ...
    const { data: profile } = await supabase.from('profiles').select('onboarding_done').eq('id', data.user.id).maybeSingle();
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0a0a0a', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
      <div style={{ width: '100%', maxWidth: 380 }}>
        ...
        <h2 style={{ fontSize: 28, fontWeight: 900, color: '#fff', marginBottom: 32, letterSpacing: '-.025em' }}>Bon retour.</h2>
        ...
        <p style={{ textAlign: 'center', marginTop: 24, fontSize: 13, color: '#444' }}>
          ...
          <Link to="/register" style={{ color: '#c8ff00', textDecoration: 'none', fontWeight: 700 }}>Créer un compte</Link>
        </p>
      </div>
    </div>
  );
